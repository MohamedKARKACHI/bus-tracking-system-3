...
export default function BookTicketPage() {
  ...
  useEffect(() => {
    const fetchRoutes = async () => {
      try {
        setLoading(true)
        const response = await fetch('http://localhost:4000/api/bookings/routes')
        if (response.ok) {
    ...
    const fetchPopularRoutes = async () => {
      try {
        const response = await fetch('http://localhost:4000/api/bookings/routes/popular/list')
        if (response.ok) {
          const data = await response.json()
  ...
  // Search routes when from/to changes
  useEffect(() => {
    if (booking.from || booking.to) {
      const searchRoutes = async () => {
        try {
          ...
          const response = await fetch(`http://localhost:4000/api/bookings/routes/search?${params}`)
          if (response.ok) {
            const data = await response.json()
            setRoutes(data)
          }
  ...
  const handlePayment = async () => {
    try {
      const response = await fetch('http://localhost:4000/api/bookings/bookings', {
        method: 'POST',
      ...
      if (response.ok) {
        const data = await response.json();
        router.push(`/client-portal/my-tickets/${data.ticketId}`);
      }
    } catch (error) {
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <Button 
                  ...
                  onClick={() => {/* Routes will filter automatically */}}
                >
